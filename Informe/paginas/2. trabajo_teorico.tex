...
% 	\draw (0, 3) to[american inductor, l={$L_{FC}$}, label distance=0.02cm] (3, 3);
% 	\draw (3, 3) to[american resistor, l={$R_{FC}$}, label distance=0.02cm] (6, 3);
% 	\draw (6, 3) to[capacitor, l_={$C_L                   $}, label distance=0.02cm] (6, -0);
% 	\draw (0, -0) -- (6, -0);
% 	\draw node[vcc, rotate=-90] (N1) at (2.75, 3) {} node[anchor=south east] at ([xshift=0.62cm, yshift=-0.62cm]N1.text){$I_{FC}$};
% 	\draw (7, 2.75) -| (7, 2.25) |- (6.75, 2.5) -- (7.25, 2.5);
% 	\draw (6.75, 0.5) -- (7.25, 0.5);
% 	\node[shape=rectangle, inner sep=0, minimum width=0.465cm, minimum height=0.465cm](Rect1) at (7, 1.5){} node[anchor=center] at (Rect1.center){$V_{CL}$};
% 	\node[shape=rectangle, draw, line width=1pt, inner sep=0, minimum width=-0.035cm, minimum height=-0.035cm] at (7, 1.5){};
% 	\node[shape=circle, draw, line width=1pt, inner sep=0, minimum width=0.298cm] at (7, 3){};
% 	\node[shape=circle, draw, line width=1pt, inner sep=0, minimum width=0.298cm] at (7, -0){};
% 	\draw (6, -0) -- (6.833, -0);
% 	\draw (6, 3) -- (6.833, 3);
% \end{tikzpicture}
% \end{center}
...
% \begin{tikzpicture}
% 	% Paths, nodes and wires:
% 	\draw (1, 3) to[capacitor, l={$C_L$}, label distance=0.02cm, mirror] (1, -0);
% 	\draw (7, 0) -| (1, -0);
% 	\draw node[vcc, rotate=-90, yscale=-1] (N1) at (4.25, 3) {} node[anchor=south east] at ([xshift=0.62cm, yshift=-0.62cm]N1.text){$I_{B}$};
% 	\draw (-0, 2.75) -| (-0, 2.25) |- (0.25, 2.5) -- (-0.25, 2.5);
% 	\draw (0.25, 0.5) -- (-0.25, 0.5);
% 	\node[shape=rectangle, inner sep=0, minimum width=0.465cm, minimum height=0.465cm, rotate=-180](Rect1) at (-0, 1.5){} node[anchor=center] at (Rect1.center){$V_{CL}$};
% 	\node[shape=rectangle, draw, line width=1pt, inner sep=0, minimum width=-0.035cm, minimum height=-0.035cm] at (7, 1.5){};
% 	\node[shape=circle, draw, line width=1pt, inner sep=0, minimum width=0.298cm, rotate=-180] at (-0, 3){};
% 	\node[shape=circle, draw, line width=1pt, inner sep=0, minimum width=0.298cm, rotate=-180] at (-0, -0){};
% 	\draw (1, 0) -| (0.167, -0);
% 	\draw (1, 3) -- (0.167, 3);
% 	\draw (4, 3) to[american inductor, l_={$L_{B}$}, label distance=0.02cm, mirror] (1, 3);
% 	\draw (7, 3) to[american resistor, l_={$R_{B}$}, label distance=0.02cm, mirror] (4, 3);
% 	\draw (7, 3) to[battery, l={$V_{B}$}, label distance=0.02cm] (7, -0);
% \end{tikzpicture}
% \end{center}
